...
class UnitFactory {
  ...
  static UnitHandle createUnit(Class<?> unitClass, boolean main) {
    ...
    UnitWrapper unitInstance = createUnitInstance(unitClass, unitWrapperClass, unitHandle);
    loadUnitHandle(unitHandle, unitInstance);
    unitHandle.setMain(main);
    return unitHandle;
  }

  static <U> UnitWrapper createUnitInstance(Class<U> unitClass, Class<?> wrapperClass, UnitHandle unitHandle) {
    try {
  ...
  public static void loadUnitHandle(UnitHandleImpl unitHandle, UnitWrapper unitInstance) {
    UnitType unitType = unitInstance.$unitType();
    Class<?> unitClass = unitType.unitClass();
    List<UnitMethod> unitMethods = unitType.methods();

    unitHandle.setUnitClass(unitClass);
    unitHandle.setUnitInstance(unitInstance);
    unitHandle.setStartupAction(buildStartupAction(unitInstance, unitClass, unitMethods));
    unitHandle.setShutdownAction(buildShutdownAction(unitInstance, unitClass, unitMethods));
    unitHandle.setInjections(buildUnitInjections(unitClass, unitMethods));
    unitHandle.setProjectionDefinitions(buildUnitProjectionDefinitions(unitInstance, unitClass, unitMethods));
    unitHandle.setGuideActions(buildUnitGuideActions(unitInstance, unitMethods));
    unitHandle.setGuides(buildGuides(unitInstance, unitClass, unitMethods));
  }
