...
import tech.intellispaces.reflections.framework.testsamples.system.EmptyTestUnit;
...
public class ModuleValidatorTest {
  ...
  public void testValidate_whenNoMainUnit() {
    // Given
    var unit1 = mock(UnitHandle.class);
    when(unit1.isMain()).thenReturn(false);

    var unit2 = mock(UnitHandle.class);
    when(unit2.isMain()).thenReturn(false);

    var module = mock(ModuleHandleImpl.class);
    when(module.unitHandles()).thenReturn(List.of(unit1, unit2));
  ...
  public void testValidate_whenTwoProjectionsWithSameName() throws Exception {
    ...
    Assertions.assertThatThrownBy(() -> ModuleValidator.validateModule(module))
        .isExactlyInstanceOf(ConfigurationException.class)
        .hasMessage("Found multiple projections with same name: Projection name '" + projectionName + "', projection definitions: " +
  ...
  public void testValidate_whenUnitProjectionMiss() {
    ...
    var unit = mock(UnitHandle.class);
    ...
    var module = mock(ModuleHandleImpl.class);
    when(module.unitHandles()).thenReturn(List.of(unit));
